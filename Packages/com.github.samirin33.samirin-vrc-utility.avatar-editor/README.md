...
| 機能 | メニュー | 説明 |
|------|----------|------|
| [Animator デフォルト設定](docs/features/01-animator-default-setting.md) | （Animator ウィンドウ内で自動適用） | 新規ステート・遷移・レイヤーの初期値 |
| [Behaviour コピー・ペースト](docs/features/02-animator-behaviour-copy.md) | Animator 内の Behaviour で右クリック | StateMachineBehaviour / ステートのコピー・ペースト |
...
| [VRChat Avatar Param Setter](docs/features/05-vrc-avatar-param-setter.md) | samirin33 Editor Tools → VRChat Avatar Param Setter | 不足 VRChat パラメータの一括追加 |
...
| [FPS Limiter](docs/features/07-fps-limiter-editor.md) | GameObject → SamirinEditorTools → FPS Limiter | 目標 FPS 制限用コンポーネント |
